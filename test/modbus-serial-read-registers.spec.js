            ...
            modbusSerialPort.connect().then(function(){
                ...
                setTimeout(function(){
                    if (modbusSerialPort.serialPort != null) {
                        ...
                        modbusSerialPort.serialPort.writeToComputer(Buffer.from(  [0x01, 0x03, 0x14, 0x02, 0x02, 0x02, 0x02, 0x00, 0x02,
    ...
    it("Read Input Registers Test", function (done) {
            ...
            modbusSerialPort.connect().then(function(){
                modbusSerialPort.readInputRegisters(1, 0, 10).then(function(outputBuffer){
                    ...
                    expect(outputBuffer).to.deep.equal(Buffer.from([0x01, 0x04, 0x14, 0x00, 0x00, 0x00, 0x01, 0x00, 0x02,
                ...
                setTimeout(function(){
                    if (modbusSerialPort.serialPort != null) {
                        console.log("**Got serial port object**");
                        modbusSerialPort.serialPort.writeToComputer(Buffer.from(  [0x01, 0x04, 0x14, 0x00, 0x00, 0x00, 0x01, 
